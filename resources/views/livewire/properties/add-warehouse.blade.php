<div>
    <x-form-modal maxWidth="xl" wire:model.defer="isCreateModalOpen" submit="">
        ...
        <x-slot name="content">
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-12">
                <div class="col-span-12">
                    <x-label :for="__('note1')" :value="__('Note 1')" :errors="$errors"/>
                    <div class="relative">
                        <x-input id="area" class="block w-full mt-1" type="text" :name="__('note1')" :value="old('note1')" :errors="$errors" 
                                wire:model.defer="note1"
                                wire:loading.attr="disabled"
                                wire:target='saveWarehouse'
                                required
                                autofocus
                        />
                        <x-input-error for="note1"/>
                    </div>
                </div>
                <div class="col-span-full">
                    <x-label :for="__('note2')" :value="__('Note 2')" :errors="$errors"/>
                    <div class="relative">
                        <x-input id="note2" class="block w-full mt-1" type="text" :name="__('note2')" :value="old('note2')" :errors="$errors" 
                                wire:model.defer="note2"
                                wire:loading.attr="disabled"
                                wire:target='saveWarehouse'
                                required
                                autofocus
                        />
                        <x-input-error for="note2"/>
                    </div>
                ...
                {{-- <div class="col-span-12">
                    ...
                    <div class="relative">
                        <x-input id="email" class="block w-full mt-1" type="email" :name="__('email')" :value="old('email')" :errors="$errors" 
        ...
        <x-slot name="footer">
            {{-- <x-button type="button" wire:click="closeAddModal()" wire:loading.attr="disabled" wire:target="store, update" class="my-1 ml-3">
                <x-icons.solid.x class="w-4 h-4 mr-2"/>
                {{ __('Cancel') }}
            </x-button> --}}
            <x-button class="my-1 ml-3" wire:click="saveWarehouse" wire:loading.attr="disabled" wire:targe='saveWarehouse'>
                <div wire:loading wire:target="">
                    <x-icons.loading class="w-4 mr-2"/>
                </div>
                <x-icons.solid.paper-plane wire:loading.remove wire:target="saveWarehouse" class="w-4 h-4 mr-1 -mt-1 rotate-45"/>
                {{ __('Save') }}
